...
<x-marketing title="Trading Rules" description="Evaluation targets, drawdown limits, and prohibited & allowed activities.">
    <section class="mx-auto max-w-4xl px-6 py-16">
        <div class="text-center">
            <h1 class="font-display text-4xl font-extrabold text-white">Trading rules</h1>
            <p class="mx-auto mt-3 max-w-xl text-slate-400">Everything you need to pass — clearly laid out, no hidden rules.</p>
        </div>

        <div x-data="{ type: '{{ $firstType }}', tab: 'conditions' }" class="mt-10">
            {{-- Challenge-type tabs --}}
            ...
            {{-- Sub-tabs --}}
            <div class="mt-6 flex justify-center gap-1 border-b border-ink-800">
            ...
            {{-- Trading conditions (per type) --}}
            <div x-show="tab === 'conditions'" x-cloak class="mt-6">
                    ...
                    <div x-show="type === '{{ $key }}'" x-cloak>
                        <div class="grid gap-3 sm:grid-cols-2">
                            ...
                            @foreach ($rows as $label => $value)
                                <div class="flex items-center justify-between rounded-xl border border-ink-700 bg-ink-900 px-4 py-3">
            ...
            {{-- Prohibited --}}
            <div x-show="tab === 'prohibited'" x-cloak class="mt-6 space-y-3">
                @forelse ($prohibited as $rule)
                    <details class="group rounded-xl border border-ink-700 bg-ink-900 px-5 py-4">
                        <summary class="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-slate-100">
                            <span class="flex items-center gap-2">
                                <svg class="h-4 w-4 text-red-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                                {{ $rule['title'] }}
                            </span>
                ...
                @empty
                    <p class="text-center text-sm text-slate-500">No prohibited activities configured.</p>
                @endforelse
            </div>

            {{-- Allowed --}}
            <div x-show="tab === 'allowed'" x-cloak class="mt-6 space-y-3">
                @forelse ($allowed as $rule)
                    <div class="rounded-xl border border-ink-700 bg-ink-900 px-5 py-4">
                        <h3 class="flex items-center gap-2 text-sm font-semibold text-slate-100">
                            <svg class="h-4 w-4 text-brand-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                            {{ $rule['title'] }}
                        </h3>
                        <p class="mt-1.5 pl-6 text-sm text-slate-400">{{ $rule['body'] }}</p>
                    </div>
                @empty
                    <p class="text-center text-sm text-slate-500">No allowed activities configured.</p>
                @endforelse
            </div>
        </div>

        <div class="mt-12 text-center">
            <a href="{{ route('register') }}" class="inline-flex rounded-lg bg-brand-500 px-6 py-3 font-semibold text-ink-950 transition hover:bg-brand-400">Start a challenge</a>
        </div>
    </section>
</x-marketing>
